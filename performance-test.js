...
async function testLoginPerformance() {
  ...
  const result = await testEndpoint('Login Page Load', `${TEST_CONFIG.baseUrl}/login`);

  const isWithinLimit = result.duration <= TEST_CONFIG.maxResponseTime.login;

  if (isWithinLimit) {
    console.log(
      `✅ Login performance: PASS (${result.duration}ms <= ${TEST_CONFIG.maxResponseTime.login}ms)`
    );
  } else {
    console.log(
      `⚠️ Login performance: SLOW (${result.duration}ms > ${TEST_CONFIG.maxResponseTime.login}ms)`
    );
  }
...
async function testAPIPerformance() {
  ...
  for (const endpoint of endpoints) {
    const result = await testEndpoint(endpoint.name, `${TEST_CONFIG.baseUrl}${endpoint.path}`);
...
async function testDatabaseOptimization() {
  ...
  const promises = Array(parallelRequests)
    .fill()
    .map(() => fetch(`${TEST_CONFIG.baseUrl}/api/pilots`));
  ...
  try {
    ...
    console.log(
      `✅ Parallel requests: ${parallelRequests} requests in ${duration}ms (avg: ${avgDuration}ms)`
    );
