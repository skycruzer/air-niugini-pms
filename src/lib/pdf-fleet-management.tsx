...
import { FleetManagementReportData, CaptainQualification, LeaveAnalysis, OperationalReadiness } from '@/types/pdf-reports'
...
export const FleetManagementPDFDocument: React.FC<FleetManagementPDFProps> = ({ reportData }) => {
  const { metadata, rosterAnalysis, captainQualifications, leaveAnalysis, operationalReadiness, upcomingRetirements, recommendations } = reportData
    ...
    <Document>
        ...
        <Section title="Fleet Roster Overview">
          <SummaryStats
            stats={[
              ...
              { label: 'Inactive Pilots', value: rosterAnalysis.inactivePilots, status: rosterAnalysis.inactivePilots > 0 ? 'expiring' : 'current' },
            ]}
          />
        </Section>
      ...
      <Page size="A4" style={pdfStyles.page}>
        ...
        <Section title="Captain Qualifications Overview">
          ...
          <PDFTable
            headers={['Captain Name', 'Employee ID', 'Line Capt', 'Training Capt', 'Examiner', 'Instructor', 'Check Airman', 'Total Quals']}
            ...
            <PDFTable
              ...
              data={captainQualifications
                ...
                .map(qual => {
                  ...
                  const status = daysRemaining < 0 ? 'EXPIRED' : daysRemaining <= 30 ? 'EXPIRING' : 'CURRENT'
        ...
        <Section title="Qualification Statistics">
          <SummaryStats
            stats={[
              { label: 'Line Captains', value: captainQualifications.filter(q => q.lineCaptain).length },
              { label: 'Training Captains', value: captainQualifications.filter(q => q.trainingCaptain).length },
              { label: 'Examiners', value: captainQualifications.filter(q => q.examiner).length },
              { label: 'Instructors', value: captainQualifications.filter(q => q.instructor).length },
              { label: 'Check Airmen', value: captainQualifications.filter(q => q.checkAirman).length },
            ]}
          />
        </Section>
                ...
                .map((qual, index) => (
                  <View key={index} style={pdfStyles.bulletItem}>
                    ...
                    <Text style={pdfStyles.bulletText}>
                      <Text style={pdfStyles.bold}>{qual.pilot.first_name} {qual.pilot.last_name}:</Text> {qual.qualificationNotes}
                    </Text>
                  </View>
                ))}
            </View>
      ...
      <Page size="A4" style={pdfStyles.page}>
        ...
        <Section title="Current Operational Readiness">
          <View style={[pdfStyles.flexRow, { marginBottom: 15 }]}>
            <View style={{ width: '48%' }}>
              <View style={pdfStyles.summaryCard}>
                ...
                <View style={{ marginTop: 8 }}>
                  <Text style={pdfStyles.bulletText}>
                    <Text style={pdfStyles.bold}>Overall Availability:</Text> {operationalReadiness.overallAvailability}%
                  </Text>
                  <Text style={pdfStyles.bulletText}>
                    <Text style={pdfStyles.bold}>Captain Availability:</Text> {operationalReadiness.captainAvailability}%
                  </Text>
                  <Text style={pdfStyles.bulletText}>
                    <Text style={pdfStyles.bold}>First Officer Availability:</Text> {operationalReadiness.firstOfficerAvailability}%
                  </Text>
                </View>
              </View>
            </View>
            ...
            <View style={{ width: '48%', marginLeft: '4%' }}>
              <View style={pdfStyles.summaryCard}>
                ...
                <View style={{ marginTop: 8 }}>
                  <Text style={pdfStyles.bulletText}>
                    <Text style={pdfStyles.bold}>Required Captains:</Text> {operationalReadiness.minimumCrewRequirement.captains}
                  </Text>
                  <Text style={pdfStyles.bulletText}>
                    <Text style={pdfStyles.bold}>Available Captains:</Text> {operationalReadiness.currentCertifiedCrew.captains}
                  </Text>
                  <Text style={pdfStyles.bulletText}>
                    <Text style={pdfStyles.bold}>Required F/Os:</Text> {operationalReadiness.minimumCrewRequirement.firstOfficers}
                  </Text>
                  <Text style={pdfStyles.bulletText}>
                    <Text style={pdfStyles.bold}>Available F/Os:</Text> {operationalReadiness.currentCertifiedCrew.firstOfficers}
                  </Text>
                </View>
              </View>
            </View>
          ...
          {(operationalReadiness.shortfall.captains > 0 || operationalReadiness.shortfall.firstOfficers > 0) && (
            <AlertBox type="critical" title="🚨 Crew Shortfall Identified">
              <Text>
                {operationalReadiness.shortfall.captains > 0 && `Captain shortfall: ${operationalReadiness.shortfall.captains}`}
                {operationalReadiness.shortfall.captains > 0 && operationalReadiness.shortfall.firstOfficers > 0 && ', '}
                {operationalReadiness.shortfall.firstOfficers > 0 && `First Officer shortfall: ${operationalReadiness.shortfall.firstOfficers}`}
              </Text>
            </AlertBox>
          )}
        </Section>
      ...
      <Page size="A4" style={pdfStyles.page}>
        ...
        {upcomingRetirements.length > 0 && (
          <Section title="Upcoming Retirements (Next 5 Years)">
            <PDFTable
              headers={['Pilot', 'Employee ID', 'Role', 'Retirement Date', 'Years Remaining', 'Replacement Plan']}
            ...
            {upcomingRetirements.filter(r => r.yearsToRetirement <= 2).length > 0 && (
              <AlertBox type="warning" title="⏰ Immediate Succession Planning Required">
                <Text>
                  {upcomingRetirements.filter(r => r.yearsToRetirement <= 2).length} pilot(s) will retire within 2 years.
                  Immediate succession planning and recruitment should be initiated.
                </Text>
              </AlertBox>
            )}
          </Section>
          ...
          <View style={pdfStyles.summaryCard}>
            ...
            <View style={{ marginTop: 8 }}>
              <Text style={[
                ...
                operationalReadiness.riskAssessment === 'LOW' ? pdfStyles.statusCurrent :
                operationalReadiness.riskAssessment === 'MEDIUM' ? pdfStyles.statusExpiring :
                operationalReadiness.riskAssessment === 'HIGH' ? pdfStyles.statusExpired :
              ...
              <Text style={pdfStyles.bulletText}>
                Fleet availability: {operationalReadiness.overallAvailability}% |
                Leave impact: {100 - leaveAnalysis.availabilityImpact.availabilityPercentage}% |
                Upcoming retirements: {upcomingRetirements.filter(r => r.yearsToRetirement <= 5).length}
              </Text>
            </View>
          </View>
        </Section>
        ...
        <View style={{ position: 'absolute', bottom: 80, left: 0, right: 0 }}>
          <AlertBox type="info" title="Report Information">
            <Text>
              • This fleet management report provides strategic insights for B767 operations planning.
              {'\n'}• Data includes active pilots, current qualifications, and approved leave requests.
              {'\n'}• Operational readiness calculated based on minimum crew requirements and current availability.
              {'\n'}• Succession planning recommendations based on retirement projections and qualification gaps.
...
export const OperationalReadinessPDFDocument: React.FC<OperationalReadinessPDFProps> = ({ operationalReadiness, metadata }) => {
  return (
    <Document>
      <Page size="A4" style={pdfStyles.page}>
        <PDFHeader metadata={{
        ...
        <Section title="Current Operational Status">
          <SummaryStats
            stats={[
              { label: 'Overall Availability', value: `${operationalReadiness.overallAvailability}%`, status: operationalReadiness.overallAvailability >= 90 ? 'current' : operationalReadiness.overallAvailability >= 75 ? 'expiring' : 'expired' },
              { label: 'Captain Availability', value: `${operationalReadiness.captainAvailability}%`, status: operationalReadiness.captainAvailability >= 90 ? 'current' : 'expiring' },
              { label: 'F/O Availability', value: `${operationalReadiness.firstOfficerAvailability}%`, status: operationalReadiness.firstOfficerAvailability >= 90 ? 'current' : 'expiring' },
              { label: 'Risk Level', value: operationalReadiness.riskAssessment, status: operationalReadiness.riskAssessment === 'LOW' ? 'current' : operationalReadiness.riskAssessment === 'MEDIUM' ? 'expiring' : 'expired' },
            ]}
          />
        </Section>
        ...
        {operationalReadiness.riskAssessment !== 'LOW' && (
          ...
          >
            <Text>
              Fleet operational readiness is currently at {operationalReadiness.riskAssessment} risk.
              {operationalReadiness.shortfall.captains > 0 && ` Captain shortfall: ${operationalReadiness.shortfall.captains}.`}
              {operationalReadiness.shortfall.firstOfficers > 0 && ` First Officer shortfall: ${operationalReadiness.shortfall.firstOfficers}.`}
            </Text>
          </AlertBox>
