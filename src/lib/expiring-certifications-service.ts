...
export async function getExpiringCertifications(daysAhead: number = 60) {
  ...
  try {
    ...
    const result = (expiringChecks || []).map((check: any) => {
      ...
      try {
        console.log('🔍 Calculating roster period for date:', expiryDate.toISOString().split('T')[0])
        ...
        if (isNaN(period.number) || !period.number || period.number <= 0 || period.code.includes('NaN')) {
          ...
          throw new Error(`Invalid roster calculation: number=${period.number}, code=${period.code}`)
        }
        ...
        console.warn('❌ Error calculating roster period for date', expiryDate.toISOString().split('T')[0], ':', error)
