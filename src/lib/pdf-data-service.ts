...
export class PDFReportDataService {
  ...
  async generateFleetManagementReportData(
    ...
    try {
      ...
      const operationalReadiness = await this.calculateOperationalReadiness(pilotsData, checksData, leaveData)
  ...
  private generateMetadata(reportType: string, title: string, generatedBy: string): PDFReportMetadata {
    return {
      reportType,
      title,
  ...
  private calculateComplianceByCategory(checks: any[], checkTypes: any[]): ComplianceByCategory[] {
    ...
    return categories.map(category => {
      ...
      const compliancePercentage = totalChecks > 0
        ? Math.round((currentChecks / totalChecks) * 100)
        : 100
  ...
  private calculatePilotComplianceStatus(pilots: any[], checks: any[], checkTypes: any[]): PilotComplianceStatus[] {
    const now = new Date()

    return pilots.filter(p => p.is_active).map(pilot => {
  ...
  private async createDetailedPilotRecord(pilot: any, checks: any[], checkTypes: any[], leaveData: any[]): Promise<DetailedPilotRecord> {
    const pilotChecks = checks.filter(check => check.pilot_id === pilot.id)
  ...
  private async calculatePilotSummary(pilot: any, checks: any[], checkTypes: any[]): Promise<PilotSummary> {
    const now = new Date()
    const currentAge = pilot.date_of_birth ? differenceInYears(now, new Date(pilot.date_of_birth)) : 0
    const totalYearsService = pilot.commencement_date ? differenceInYears(now, new Date(pilot.commencement_date)) : 0

    // Get retirement age from settings
    const settings = await this.getSettings()
    const retirementAge = settings.pilot_requirements.pilot_retirement_age
    const retirementDate = pilot.date_of_birth ? addYears(new Date(pilot.date_of_birth), retirementAge) : undefined
  ...
  private calculateFleetRosterAnalysis(pilots: any[]): FleetRosterAnalysis {
    ...
    const seniorityDistribution = seniorityRanges.map(range => ({
      ...
      count: pilots.filter(p =>
        p.seniority_number &&
        p.seniority_number >= range.min &&
        p.seniority_number <= range.max
      ).length
    }))
  ...
  private calculateLeaveAnalysis(leaveData: any[], pilots: any[]): LeaveAnalysis {
    ...
    const activePilots = pilots.filter(p => p.is_active).length
    const availabilityPercentage = Math.round(((activePilots - pilotsOnLeave) / activePilots) * 100)
  ...
  private async calculateOperationalReadiness(pilots: any[], checks: any[], leaveData: any[]): Promise<OperationalReadiness> {
    ...
    const minimumCaptains = settings.pilot_requirements.minimum_captains_per_hull * settings.pilot_requirements.number_of_aircraft
    const minimumFirstOfficers = settings.pilot_requirements.minimum_first_officers_per_hull * settings.pilot_requirements.number_of_aircraft
    ...
    const firstOfficerAvailability = Math.round((certifiedFirstOfficers / minimumFirstOfficers) * 100)
    const overallAvailability = Math.round(((certifiedCaptains + certifiedFirstOfficers) / (minimumCaptains + minimumFirstOfficers)) * 100)
    ...
    if (captainShortfall > 0) {
      mitigationActions.push(`Expedite captain certification renewals (${captainShortfall} required)`)
    }
    if (firstOfficerShortfall > 0) {
      mitigationActions.push(`Expedite first officer certification renewals (${firstOfficerShortfall} required)`)
    }
  ...
  private generateComplianceRecommendations(
    ...
    if (overview.expiredCertifications > 0) {
      recommendations.push(`Immediate action required: ${overview.expiredCertifications} certifications have expired`)
    }

    if (overview.expiringCertifications > 0) {
      recommendations.push(`Schedule renewal training for ${overview.expiringCertifications} expiring certifications`)
    }

    const highRiskPilots = pilotStatus.filter(p => p.riskLevel === 'HIGH' || p.riskLevel === 'CRITICAL').length
    if (highRiskPilots > 0) {
  ...
  private generatePilotRecommendations(pilots: DetailedPilotRecord[]): string[] {
    ...
    const nonCompliantCount = pilots.filter(p => p.summary.complianceStatus === 'NON_COMPLIANT').length
    ...
    if (atRiskCount > 0) {
      recommendations.push(`${atRiskCount} pilot(s) have expiring certifications - schedule renewals`)
    }
  ...
  private generateFleetManagementRecommendations(
    ...
    if (readiness.riskAssessment !== 'LOW') {
      recommendations.push(`Fleet operational readiness at ${readiness.riskAssessment} risk - immediate attention required`)
    }
