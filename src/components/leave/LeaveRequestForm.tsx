...
import { createLeaveRequest, updateLeaveRequest, checkLeaveConflicts, type LeaveRequestFormData, type LeaveRequest } from '@/lib/leave-service'
import { getCurrentRosterPeriod, getRosterPeriodFromDate, getAffectedRosterPeriods } from '@/lib/roster-utils'
...
export function LeaveRequestForm({ onSuccess, onCancel, editingRequest }: LeaveRequestFormProps) {
  console.log('🔧 LeaveRequestForm mounted/updated with editingRequest:', editingRequest?.id, editingRequest?.pilot_id)
  ...
  const onSubmit = async (data: LeaveRequestFormData) => {
    ...
    try {
      ...
      setError('root', {
        type: 'manual',
        message: error instanceof Error ? error.message : `Failed to ${editingRequest ? 'update' : 'create'} leave request`
      })
    } finally {
      setLoading(false)
    }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-sm text-gray-600 mt-1">
          ...
          {affectedRosterPeriods.length > 0 && (
            <div className="sm:ml-auto">
              Request affects: <span className="font-medium text-blue-600">
