...
export default function CertificationExpiryPlanningPage() {
  ...
  const handleEmailToTraining = async () => {
    ...
    try {
      ...
      const subject = encodeURIComponent(`Air Niugini B767 Certification Expiry Planning - ${timeframeLabel}`)
...
${Object.entries(state.stats?.byCategory || {}).map(([category, count]) => `- ${category}: ${count} certifications`).join('\n')}
...
Recipients: ${recipients.split(',').join(', ')}`)
  ...
  const groupedCertifications = state.expiringCertifications.reduce((groups, cert) => {
  ...
  return (
    <ProtectedRoute>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Certification Expiry Planning</h1>
              <p className="text-gray-600">Monitor and plan for upcoming certification expirations</p>
            </div>
          </div>
      ...
      {state.stats && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Certification Expiry Overview</h2>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-[#E4002B]">{state.stats.totalExpiring}</div>
              <div className="text-sm text-gray-600">Total Expiring</div>
            </div>

            <div className="bg-red-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-red-600">{state.stats.byStatus.expired || 0}</div>
              <div className="text-sm text-gray-600">Already Expired</div>
            </div>

            <div className="bg-yellow-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-yellow-600">{state.stats.byStatus.expiring_soon || 0}</div>
              <div className="text-sm text-gray-600">Expiring Soon (≤30 days)</div>
            </div>

            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-600">{state.stats.byStatus.upcoming || 0}</div>
          ...
          <div>
            <h3 className="text-md font-medium text-gray-900 mb-3">Expiring by Roster Period</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
              {Object.entries(state.stats.byRosterPeriod)
                .sort(([a], [b]) => a.localeCompare(b))
                .map(([rosterPeriod, count]) => (
                <div key={rosterPeriod} className="bg-[#E4002B]/5 border border-[#E4002B]/20 rounded p-2 text-center">
          ...
          {state.expiringCertifications.length === 0 ? (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12">
              <div className="text-center">
                ...
                <h3 className="mt-4 text-lg font-medium text-gray-900">No Expiring Certifications</h3>
                  ...
                  <div className="p-4 border-b border-gray-200">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-semibold text-gray-900 flex items-center">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mr-3 ${getCategoryBadgeClass(category)}`}>
                          {category}
                        </span>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {certifications
                          ...
                          <tr key={cert.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${cert.status.className}`}>
                                {cert.status.label}
                              </span>
                            </td>
                          </tr>
