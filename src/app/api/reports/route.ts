...
async function generateRiskAssessmentReport(supabaseAdmin: any) {
  ...
  const reportData = {
    summary: {
      ...
      riskLevel: (expiredData?.length || 0) > 10 ? 'HIGH' :
                 (expiredData?.length || 0) > 5 ? 'MEDIUM' : 'LOW'
    },
    expiredCertifications: expiredData?.map((item: any) => ({
      ...
      daysOverdue: Math.floor((Date.now() - new Date(item.expiry_date).getTime()) / (1000 * 60 * 60 * 24))
    })),
    expiringCertifications: expiringData?.map((item: any) => ({
      ...
      category: item.check_type?.category,
      expiryDate: item.expiry_date,
      daysUntilExpiry: Math.floor((new Date(item.expiry_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
    })),
    generatedAt: new Date().toISOString(),
    generatedBy: 'Air Niugini PMS'
  }
    ...
    pilots: pilots?.map((pilot: any) => {
      ...
      return {
        ...
        certifications: certifications.map((cert: any) => ({
          ...
          status: !cert.expiry_date ? 'EXPIRED' :
                  new Date(cert.expiry_date) <= today ? 'EXPIRED' :
                  new Date(cert.expiry_date) <= new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000) ? 'EXPIRING' : 'CURRENT'
        }))
      }
    }),
    generatedAt: new Date().toISOString(),
...
async function generateFleetAnalyticsReport(supabaseAdmin: any) {
  ...
  const { data: checkTypes } = await supabaseAdmin
    .from('check_types')
    .select('*')
...
function calculateAverageSeniority(pilots: any[]): number {
  ...
  const seniorities = pilots
    .filter(p => p.seniority_number)
    .map(p => p.seniority_number)
...
async function generatePlanningRosteringReport(supabaseAdmin: any) {
  ...
  for (const period of timePeriods) {
    ...
    expiring?.forEach((item: any) => {
      ...
      pilotImpact[pilotKey].certifications.push({
        ...
        category: item.check_type?.category,
        expiryDate: item.expiry_date,
        daysUntilExpiry: Math.floor((new Date(item.expiry_date).getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
      })
    ...
    periodData[period.label] = {
      ...
      criticalPilotsAffected: Object.values(pilotImpact).filter((pilot: any) => pilot.criticalChecks > 0).length,
  ...
  const captainsAffected = Object.values(periodData['28 Days'].pilotImpact)
    .filter((pilot: any) => pilot.pilot.role === 'Captain' && pilot.criticalChecks > 0)

  const firstOfficersAffected = Object.values(periodData['28 Days'].pilotImpact)
    .filter((pilot: any) => pilot.pilot.role === 'First Officer' && pilot.criticalChecks > 0)
  ...
  const operationalImpact = {
    next28Days: {
      ...
      riskLevel: captainsAffected.length > 3 ? 'HIGH' :
                captainsAffected.length > 1 ? 'MEDIUM' : 'LOW',
      recommendations: generatePlanningRecommendations(captainsAffected.length, firstOfficersAffected.length)
    }
  }
  ...
  const currentAvailability = {
    ...
    total: (currentCaptains + currentFirstOfficers) - (leaveImpactAnalysis.captainsOnLeave + leaveImpactAnalysis.firstOfficersOnLeave)
  }
  ...
  const pilotRequirementsAnalysis = {
    ...
    retirementRisk: {
      ...
      percentageNearRetirement: allPilots?.length ? Math.round((pilotsNearRetirement / allPilots.length) * 100) : 0,
      retirementAge
    },
    aircraftCoverage: {
      numberOfAircraft,
      currentCoverageRatio: numberOfAircraft > 0 ? Math.round(((currentCaptains + currentFirstOfficers) / numberOfAircraft) * 100) / 100 : 0,
      requiredCoverageRatio: numberOfAircraft > 0 ? Math.round(((requiredCaptains + requiredFirstOfficers) / numberOfAircraft) * 100) / 100 : 0
    }
  }
  ...
  const reportData = {
    summary: {
      ...
      criticalPeriod: periodData['28 Days'].criticalPilotsAffected > 0 ? '28 Days' :
                     periodData['14 Days'].criticalPilotsAffected > 0 ? '14 Days' :
                     periodData['7 Days'].criticalPilotsAffected > 0 ? '7 Days' : 'None'
    },
    ...
    rosterAnalysis: {
      ...
      availabilityPercentage: {
        captains: currentCaptains > 0 ? Math.round((currentAvailability.captains / currentCaptains) * 100) : 100,
        firstOfficers: currentFirstOfficers > 0 ? Math.round((currentAvailability.firstOfficers / currentFirstOfficers) * 100) : 100,
        overall: (currentCaptains + currentFirstOfficers) > 0 ? Math.round((currentAvailability.total / (currentCaptains + currentFirstOfficers)) * 100) : 100
      }
    },
    planningInsights: {
...
function generatePlanningRecommendations(captainsAtRisk: number, firstOfficersAtRisk: number): string[] {
...
function generateTrainingPriority(periodData: any): Array<{category: string, priority: string, reason: string}> {
