...
test.describe('Task Management - Basic Operations', () => {
  ...
  test('should display tasks dashboard', async ({ page }) => {
    ...
    const viewToggle = page.locator('button:has-text("List"), button:has-text("Kanban"), button:has-text("Grid")');
  ...
  test('should create a new task', async ({ page }) => {
    ...
    await page.click('button:has-text("New Task"), button:has-text("Create"), button:has-text("Add")');
    ...
    await expect(page.locator('text=/created.*success/i, text=/success/i')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should add checklist items to task', async ({ page }) => {
    ...
    // Add checklist item
    const checklistInput = page.locator('input[name="checklist_item"], input[placeholder*="checklist" i]');
...
test.describe('Task Management - Search and Sort', () => {
  ...
  test('should search tasks by title', async ({ page }) => {
    ...
    if (await searchInput.count() > 0) {
      ...
      const hasSearchTerm = taskTitles.some(title => title.toLowerCase().includes('certification'));
...
test.describe('Task Management - Data Integrity', () => {
  ...
  test('should preserve task data after navigation', async ({ page }) => {
    ...
    const firstTask = await page.locator('table tbody tr:first-child, .task-card:first-child').textContent();
